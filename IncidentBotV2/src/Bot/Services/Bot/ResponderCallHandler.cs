...
namespace Sample.IncidentBot.Bot
{
    ...
    public class ResponderCallHandler : CallHandler
    {
        ...
        public ResponderCallHandler(ICall call, BotService botService, string responderId, IncidentStatusData statusData)
            : base(call, botService)
        {
        ...
        private void PlayTransferingPrompt()
        {
            Task.Run(async () =>
            {
                try
                {
                    await this.Call.PlayPromptAsync(new List<MediaPrompt> { this.BotInstance.MediaMap[BotConstants.TransferingPromptName] }).ConfigureAwait(false);
        ...
        private void PlayNotificationPrompt()
        {
            Task.Run(async () =>
            {
                try
                {
                    await this.Call.PlayPromptAsync(new List<MediaPrompt> { this.BotInstance.MediaMap[BotConstants.NotificationPromptName] }).ConfigureAwait(false);
            ...
            Task.Run(async () =>
            {
                try
                {
                    ...
                    if (incidentMeetingCallId != null && responderStatusData != null)
                    {
                        ...
                        await this.BotInstance.AddParticipantAsync(incidentMeetingCallId, addParticipantRequestData).ConfigureAwait(false);
