...
namespace Sample.IncidentBot.Bot
{
    ...
    public class IncomingCallHandler : CallHandler
    {
        ...
        public IncomingCallHandler(ICall call, BotService botService, string endpointId)
        ...
        private void PlayNotificationPrompt()
        {
            Task.Run(async () =>
            {
                try
                {
                    var mediaName = this.endpointId == null ? BotConstants.BotIncomingPromptName : BotConstants.BotEndpointIncomingPromptName;

                    await this.Call.PlayPromptAsync(new List<MediaPrompt> { this.BotInstance.MediaMap[mediaName] }).ConfigureAwait(false);
