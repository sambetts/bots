...
namespace TranslatorBot.Services.Http.Controllers
{
    ...
    public class JoinCallController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> JoinCallAsync([FromBody] JoinCallRequestData joinCallBody)
        {
            try
            {
                _logger.LogInformation("JOIN CALL");
                var body = await this.Request.Content.ReadAsStringAsync();
                var call = await _botService.JoinCallAsync(joinCallBody, string.Empty).ConfigureAwait(false);
