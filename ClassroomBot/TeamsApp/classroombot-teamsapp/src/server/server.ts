...
express.get("/exchangeSsoTokenForOboToken", async (req, res) => {
    ...
    const clientId = process.env.MICROSOFT_APP_ID as string;
    const clientSecret = process.env.MICROSOFT_APP_PASSWORD as string;
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if ((error.response.data.error === "invalid_grant") || (error.response.data.error === "interaction_required")) {
            // if consent required... reply with 403: Forbidden
            res.status(403).json({ error: "consent_required" });
          } else {
            // else, some other error occurred... fail
            res.status(500).json({ error: "Could not exchange access token: " + error.response.data.error_description });
          }
    }
  });
